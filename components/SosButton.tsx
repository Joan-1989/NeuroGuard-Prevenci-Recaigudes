...
const SosButton: React.FC<SosButtonProps> = ({ profileType }) => {
  ...
  useEffect(() => {
    ...
    if (active && timeLeft > 0) {
      ...
      setActive(false);
      alert("Felicitats! Has superat l'impuls (Urge Surfing).");
      setTimeLeft(180);
    }
  ...
  return (
    <>
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 text-white font-bold py-3 px-6 rounded-full transition-all transform hover:scale-105 ${getStyles()}`}
      >
        {getLabel()}
      </button>

      {active && (
        <div className="fixed inset-0 z-[60] flex flex-col items-center justify-center bg-black/90 backdrop-blur-md text-white p-6">
